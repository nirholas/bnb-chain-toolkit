 ...
 * 
 * No external dependencies beyond solc itself - 100% BNB Chain AI Toolkit owned code.
 */
...
/**
 * BNBCompiler - Browser-based Solidity compilation
 */
...
export const bnbCompiler = new BNBCompiler();
...
export async function compileCode(source: string, options?: Partial<CompileInput>): Promise<CompileOutput> {
  return bnbCompiler.compile({ source, ...options });
}
