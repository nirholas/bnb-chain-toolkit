 ...
 * AI Chat Components
 * 
 * Self-contained chat components with BNB Chain branding.
 * Uses Tailwind CSS for styling - no external UI library dependencies.
 */

import React, { memo, useState, useCallback } from 'react';
import { BNBLogo } from './BNBBrand';
...
/**
 * Pre-styled chat message with AI/User defaults
 */
export const AIChatMessage = memo<AIChatMessageProps>(({ 
  content,
  isAI = false, 
  loading = false,
  timestamp,
}) => {
  const avatar = isAI ? BNB_AI_AVATAR : USER_AVATAR;
 ...
 */
export const AIChatPanel = memo<AIChatPanelProps>(({
  messages = [],
  onSend,
  loading = false,
  placeholder = 'Ask about BNB Chain...',
  title = 'AI Assistant',
  className = '',
}) => {
      ...
      <div className="flex-1 overflow-auto p-4 space-y-4">
        {messages.length === 0 ? (
          <div className="flex-1 flex items-center justify-center h-full text-gray-400">
            <p>Start a conversation with BNB Chain AI</p>
          </div>
        ) : (
          messages.map((msg) => (
            <AIChatMessage
