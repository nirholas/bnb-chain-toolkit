...
**Agenti** is deployed and accessible over HTTP via [MCP Streamable HTTP](https://modelcontextprotocol.io/specification/2025-03-26/basic/transports#streamable-http) transport — no local installation required.

**Endpoint:**
```
https://modelcontextprotocol.name/mcp/agenti
```
...
```json
{
  "mcpServers": {
    "agenti": {
      "type": "http",
      "url": "https://modelcontextprotocol.name/mcp/agenti"
    }
  }
}
```
...
| `get_top_yields` | Top DeFi yields |
| `get_fear_greed_index` | Fear & Greed Index |
| `get_latest_news` | Latest crypto news |
| `get_gas_prices` | Ethereum gas prices |
| `get_token_by_contract` | Token by contract |
| `get_global_tvl` | Global DeFi TVL |

### Example Requests

**Get crypto prices:**
```bash
curl -X POST https://modelcontextprotocol.name/mcp/agenti \
